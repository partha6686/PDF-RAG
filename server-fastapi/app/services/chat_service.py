...
class ChatService:
    ...
    async def generate_response(
        ...
        try:
            ...
            logger.info(f"Found {len(relevant_chunks)} relevant chunks for query: '{user_message}'")

            if not relevant_chunks:
                logger.warning(f"No relevant chunks found for document {document_id} and query: '{user_message}'")
                return {
            ...
            return {
                ...
                "sources": sources,
                "context_used": True,
                "context_chunks": relevant_chunks  # Return full EmbeddingResult objects
            }
            ...
            return {
                "response": f"I apologize, but I encountered an error while processing your question: {str(e)}. Please check the server logs for more details.",
                "sources": [],
                "context_used": False,
                "error": str(e)
            }
    ...
    def _build_prompt(self, user_message: str, context: str, history: str) -> str:
        ...
        prompt_parts.append("""You are an intelligent document assistant specialized in answering questions based on uploaded PDF documents. You are having a conversation with a user about the document content.
...
6. Use a helpful and conversational tone
7. Reference previous parts of the conversation when relevant
8. Build upon previous questions and answers to provide better context
...
---""")
        ...
        prompt_parts.append("Please provide a helpful answer based on the document context and previous conversation. If the information is not in the context, clearly state that you cannot find it in the uploaded document.")
    ...
    async def generate_response_stream(
        ...
        try:
            logger.info(f"Generating streaming response for document {document_id}")
            ...
            if not relevant_chunks:
                ...
                yield {
                    "type": "error",
                    "content": "I cannot find any relevant information in the uploaded document to answer your question. The document may not have been processed yet or may not contain information related to your query.",
                    "sources": [],
                    "context_chunks": []
                }
                return
            ...
            for chunk in response_stream:
                if chunk.text:
                    # Split text into smaller chunks for better streaming effect
            ...
            logger.error(f"Streaming chat response generation failed: {e}")
            import traceback
            logger.error(f"Full traceback: {traceback.format_exc()}")
            yield {
                "type": "error",
                "content": f"I apologize, but I encountered an error while processing your question: {str(e)}. Please check the server logs for more details.",
                "sources": [],
                "context_chunks": []
            }
