...
from app.services.process_tracker import process_tracker, ProcessStatus
...
class DocumentProcessor:
    ...
    async def process_document_with_id(self, document_id: str, process_id: str) -> Dict[str, Any]:
        """Process a document with process ID tracking"""
        try:
            # Create process in tracker
            await process_tracker.create_process(process_id, document_id)
            logger.info(f"Starting document processing for {document_id} with process {process_id}")
            ...
            try:
                ...
                document.processing_status = "processing"
                db.commit()
                await process_tracker.update_process(process_id, ProcessStatus.PROCESSING, 5, "📄 Extracting text from PDF...")
                logger.info(f"Document {document_id} status updated to processing")
                ...
                # Step 2: Extract text from PDF
                logger.info(f"Step 2: Extracting text from PDF")
                await process_tracker.update_process(process_id, ProcessStatus.PROCESSING, 10, "📄 Extracting text from PDF...")
                text_content = self.pdf_service.extract_text_from_bytes(file_data)
                ...
                # Step 3: Create text chunks
                logger.info(f"Step 3: Creating text chunks")
                await process_tracker.update_process(process_id, ProcessStatus.PROCESSING, 20, "✂️ Chunking text into segments...")
                chunks = self.pdf_service.create_text_chunks(
                    text_content,
                    chunk_size=settings.CHUNK_SIZE,
                    chunk_overlap=settings.CHUNK_OVERLAP
                ...
                # Step 4: Generate embeddings
                logger.info(f"Step 4: Generating embeddings for {len(chunks)} chunks")
                await process_tracker.update_process(process_id, ProcessStatus.PROCESSING, 40, f"🧠 Generating embeddings for {len(chunks)} chunks...")
                chunk_texts = [chunk["text"] for chunk in chunks]
                embeddings = await self.embedding_service.generate_embeddings(chunk_texts)
                ...
                # Step 5: Store in Qdrant
                logger.info(f"Step 5: Storing embeddings in Qdrant")
                await process_tracker.update_process(process_id, ProcessStatus.PROCESSING, 80, "💾 Storing chunks in vector database...")
                stored_count = await self.qdrant_service.store_document_chunks(
                    user_id=document.user_id,
                    document_id=document_id,
                    chunks=chunks,
                ...
                await process_tracker.update_process(process_id, ProcessStatus.COMPLETED, 100, "🎉 PDF processing completed successfully!")
                logger.info(f"Document {document_id} processed successfully: {stored_count} chunks stored")
                ...
                await process_tracker.update_process(process_id, ProcessStatus.FAILED, 0, f"❌ Processing failed: {str(e)}", str(e))
                logger.error(f"Document processing failed for {document_id}: {e}")
                raise
